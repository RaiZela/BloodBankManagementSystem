...
public class BloodBankService : IBloodBankService
{
    ...
    public async Task<ApiResponse<bool>> Update(BloodBankViewModel value)
    {
        try
        {
            _repository.Update<Bank>(_mapper.Map<Bank>(value));
            await _repository.SaveAsync();
            return ApiResponse<bool>.ApiOkResponse(true);
        }
        catch (Exception ex)
    ...
    public async Task<ApiResponse<List<BloodBankViewModel>>> GetAll()
    {
        try
        {
            var response = await _repository.GetQueryable<Bank>().ToListAsync();
    ...
    public async Task<ApiResponse<BloodBankViewModel>> Get(int id)
    {
        try
        {
            var response = await _repository.GetQueryable<Bank>(x => x.ID == id).FirstOrDefaultAsync();
            if (response == null)
